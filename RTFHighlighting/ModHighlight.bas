...
'Highlighting using the Richtextbox control
'is awkward. Whilst there are a host of Selection properties
'there is no Highlight property we can access from VB.
'We just have to parse the RTF code.
'It would be so much easier to just use...
'    RTF.SelHighlightColor = vbGreen
'    RTF.SelHighlight = True
'I dont think MS wanted us to use Highlighting

'If you want more information on the Rich Text Format try...
'http://msdn.microsoft.com/library/default.asp?url=/library/en-us/dnrtfspec/html/rtfspec.asp


'Anyway this little Module gets you started into
'the world of Rich Text Format and gives you
'some pretty useful functions to begin with.

'As with all my submissions, all errors and
'bugs come entirely free of charge.
...
Public Sub SelectBelow(mForm As Form, mRTF As RichTextBox, Optional DontLock As Boolean)
    ...
    If Not DontLock Then LockWindowUpdate 0
End Sub
Public Sub KeyColor(mForm As Form, mRTF As RichTextBox, mColor As Long, KeyWords() As String, Optional DontLock As Boolean)
    Dim re As New RegExp, D As Long
    ...
    re.IgnoreCase = False
    re.Global = True
    'use Regular Expressions to find each keyword
    'then use standard ".selcolor" to change the colors
    ...
    For D = LBound(KeyWords) To UBound(KeyWords)
        ...
        If Trim(KeyWords(D)) <> "" Then
            ...
            Set founditems = re.Execute(mRTF.Text)
            For Each Found In founditems
                DoEvents
                If Found <> "" Then
                    mRTF.SelStart = Found.FirstIndex
                    mRTF.SelLength = Len(re.Pattern) - 4
                    mRTF.SelColor = mColor
                End If
            Next
        End If
    Next D
